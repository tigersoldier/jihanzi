...
vi.mock('../data/db', async () => {
  ...
  return {
    ...actual,
    getFirstReviewDays: vi.fn(() => Promise.resolve(new Map())),
...
const SM2_A: SM2State = { ease: 2.5, interval: 1, repetitions: 1, nextReview: '2026-01-02', lastGrade: 'a' }

function makeState(overrides?: Partial<AppState>): AppState {
  return {
    children: [{ id: 'child_1', name: '小明', wordBookId: 'wb_1', nextCharIndex: 1,
      progress: { '雨': SM2_A } }],
...
describe('useHistory', () => {
  ...
  it('calls getFirstReviewDays on initial mount', () => {
    renderHook(() => useHistory('child_1', '2026-06'), {
      wrapper: ({ children }: { children: ReactNode }) => {
        const ctx: AppContextState = { state: makeState(), loading: false, dataVersion: 0,
  ...
  it('skips getFirstReviewDays when past month sees only dataVersion bump', async () => {
    ...
    function W({ children }: { children: ReactNode }) {
      ...
      setDV = set
      const ctx: AppContextState = { state: makeState(), loading: false, dataVersion: dv,
        selectedChildId: 'child_1', setSelectedChildId: vi.fn(), reloadState: vi.fn(),
        createChild: vi.fn() as any, updateChild: vi.fn() as any, deleteChild: vi.fn() as any,
        createWordBook: vi.fn() as any, updateWordBook: vi.fn() as any, deleteWordBook: vi.fn() as any,
        addCharacter: vi.fn() as any, removeCharacter: vi.fn() as any, reorderCharacters: vi.fn() as any,
    ...
    await vi.waitFor(() => { expect(db.getFirstReviewDays).toHaveBeenCalledTimes(1) })

    await act(async () => { setDV(1); await new Promise(r => setTimeout(r, 50)) })
    expect(db.getFirstReviewDays).toHaveBeenCalledTimes(1) // skipped
  })
})
...
describe('useCharacterStats', () => {
  ...
  it('skips re-query when dataVersion bumps but SM2State unchanged', async () => {
    ...
    // Bump dataVersion — SM2State same → should skip
    await act(async () => { setDV(1); await new Promise(r => setTimeout(r, 50)) })
    expect(db.getReviewsForChildChar).toHaveBeenCalledTimes(1)
  })
  ...
  it('re-queries when SM2State actually changes', async () => {
    ...
    function W({ children }: { children: ReactNode }) {
      ...
      setState = set
      const ctx: AppContextState = { state: s, loading: false, dataVersion: 0,
        selectedChildId: 'child_1', setSelectedChildId: vi.fn(), reloadState: vi.fn(),
        createChild: vi.fn() as any, updateChild: vi.fn() as any, deleteChild: vi.fn() as any,
        createWordBook: vi.fn() as any, updateWordBook: vi.fn() as any, deleteWordBook: vi.fn() as any,
        addCharacter: vi.fn() as any, removeCharacter: vi.fn() as any, reorderCharacters: vi.fn() as any,
