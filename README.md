...
本版本修复了"学习记录被重复计算、复习时间异常延后"的同步事故（详见
`docs/adr/0007-sm2-dedup-and-snapshot-repair.md`）：

- 未到期的重复复习不再重复计入 SM-2（日志保留）；
- 启动时对快照逐字校验修复，历史完整的污染字自动恢复正确间隔；
- 无法验证的重污染字（interval ≥ 10 年）重置为间隔 1 天，尽快重新进入复习；
- 修复后的干净快照自动推送覆盖云端污染版。

**重要：请更新所有设备的 PWA**（刷新页面完成 Service Worker 更新）。
旧版本设备（08-01 之前）会继续向云端写入污染数据——新版设备能自我修复，
但旧设备自身的显示无法远程纠正，更新是唯一根治手段。
